...
public class Catcher {
	...
	/**
	 * Starts the ping catching by opening a blocking server socket channel and
	 * listening for connections. Returns the received packet to the sender, ie.
	 * pitcher.
	 */
	public void startCatching() {
		...
		try (ServerSocketChannel serverChannel = ServerSocketChannel.open();) {
			...
			while (true) {
				...
				// record the absolute time when the pitchers ping packet
				// reached
				// the catcher, this method of using the system time is
				// unreliable because there is
				// no guarantee the clocks are synchronized on the pitcher and
				// catcher hosts
